...
//bytes -> uint32_t
uint16_t unpack_u32(const uint8_t **);

// read a defined len of bytes
uint8_t *unpack_bytes(const uint8_t, size_t, uint8_t);

// unpack a string prefixed by its length as uint16 value
uint16_t unpack_string16(uint8_t **buf, uint8_t **dest);

/* Write data on constant uint8_t pointer */
// apppend a uint8_t -> bytes into the bytestring
void pack_u8(uint8_t **, uint8_t);

// apppend a uint16_t -> bytes into the bytestring
void pack_u16(uint16_t **, uint16_t);

// apppend a uint32_t -> bytes into the bytestring
void pack_u32(uint32_t **, uint32_t);
